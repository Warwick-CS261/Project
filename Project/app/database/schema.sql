...
(
  ...
  seriesID VARCHAR(6) ,
  sname VARCHAR(30),
  mood FLOAT CHECK (mood<=1 AND mood >= -1),
  secure VARCHAR(6),
  -- security password or NULL
  userID INTEGER,
  ended BIT,
...
(
  ...
  stamp DATETIME,
  mood FLOAT,
  FOREIGN KEY(sessionID) REFERENCES SESH(id) ON DELETE CASCADE
);
...
(
  ...
  stamp DATETIME,
  -- time of reaction received (required for reaction-time diagram)
  anon BIT,
  context VARCHAR(256),
  -- context to feedback from the requirements
